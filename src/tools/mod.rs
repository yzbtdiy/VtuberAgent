...
impl ImageGenerationTool {
    ...
    pub async fn generate_image(&self, prompt: &str) -> Result<String> {
        ...
        eprintln!("Image API Response: {}", serde_json::to_string_pretty(&response_json)?);
        ...
        Err(anyhow!("No image URL found in response. Available fields: {:?}", 
...
impl TTSTool {
    ...
    pub async fn generate_speech(&self, text: &str) -> Result<Vec<u8>> {
        ...
        if !response.status().is_success() {
            let error_text = response.text().await?;
            return Err(anyhow!("TTS generation failed: {}", error_text));
        }
